...
def generate_grid_rects(
    dimensions=(6, 6), pos=(0, 0), var_x=21, var_y=71, cell_size=(32, 32)
):
...
def text(
    ...
    if font_style == "Arial":
        ...
        font = pg.font.Font(font_style, font_size)
...
class Damage:
    ...
    def draw_damage(dmg, pos1, surface, font, color, img=None, pos2=None):
        ...
        if img is not None:
            Damage.portrait_opacity -= 10
        if Damage.counter_num < Damage.movement_pos_limit * 0.5:
            Damage.num_opacity -= 10

        if img is not None:
            blit_alpha(surface, img, pos2, Damage.portrait_opacity)
...
class BlinkingText:
    ...
    @staticmethod
    def show_blinking_text(
        surface, txt, pos, font, size, color, variance, centered=True
    ):
...
class Sheet:
    ...
    def __init__(self, sheet, dimensions, transparency=True):
        self.sheet = (
            pg.image.load(sheet).convert_alpha()
            if transparency
            else pg.image.load(sheet).convert()
        )
        ...
        self.crops = list(
            [
                (
                    int(cell % self.columns) * self.crop_w,
                    int(cell / self.columns) * self.crop_h,
                    self.crop_w,
                    self.crop_h,
                )
                for cell in range(self.total_crops)
            ]
        )
...
class Button:
    ...
    def __init__(
        ...
        bg=None,
        icon=None,
        icon_pressed_custom=False,
        icon_active_custom=False,
        img=None,
        img_hover=None,
        img_pressed=None,
        img_active=None,
        ...
        if bg is not None:
            self.rect = pg.Rect(
                self.pos[0], self.pos[1], self.bg.crop_w, self.bg.crop_h
            )
        elif icon is not None:
            self.rect = pg.Rect(
                self.pos[0], self.pos[1], self.icon.crop_w, self.icon.crop_h
            )
        elif img is not None:
            self.rect = pg.Rect(
                self.pos[0], self.pos[1], self.img.get_width(), self.img.get_height()
            )
    ...
    def draw_button(self):
        ...
        self.pos_pressed = (
            self.pos[0] + self.displace_pressed[0],
            self.pos[1] + self.displace_pressed[1],
        )
        self.pos_active = (
            self.pos[0] + self.displace_active[0],
            self.pos[1] + self.displace_active[1],
        )
            ...
            if self.pressed and self.press_on:
                if self.icon_pressed_custom:
                    self.surface.blit(
                        self.icon.sheet,
                        self.pos_pressed,
                        self.icon.crops[self.sheet_index + 1],
                    )
                else:
                    self.surface.blit(
                        self.icon.sheet,
                        self.pos_pressed,
                        self.icon.crops[self.sheet_index],
                    )
            elif self.active and self.activate_on:
                if self.icon_active_custom:
                    self.surface.blit(
                        self.icon.sheet,
                        self.pos_active,
                        self.icon.crops[self.sheet_index + 2],
                    )
                else:
                    self.surface.blit(
                        self.icon.sheet,
                ...
                self.surface.blit(
                    self.icon.sheet, self.pos, self.icon.crops[self.sheet_index]
                )
