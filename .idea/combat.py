...
class Combat:
	...
	def draw_combat(self):
		...
		if self.combat_active:
			IOAtlas.fade_bg('combat', 'in', self.fading_opacity)
			if IOAtlas.opacity_counter['combat'] >= self.fading_opacity:
				IOAtlas.fading['combat'] = 'out'

			IOGUI.draw_gui(windows=False)
			...
			if IOAtlas.fading['combat'] == 'out':
				IOAtlas.fade_bg('combat', 'out')
				if IOAtlas.opacity_counter['combat'] <= 0:
					IOAtlas.fading['combat'] = 'off'
